...
Accounts.onEmailVerificationLink(function (token) {
  ...
  function verifyCallback(error) {
    if (error) Alert.error("Error in e-mail account verification: " + getErrorMessage(error))
    else Alert.success("E-mail account successfully verified.")
  }
})
...
Accounts.onLogin(function () {
  ...
  if (Meteor.user().emails && !Meteor.user().emails[0].verified)
    Alert.warning(
      "E-mail account unconfirmed. Click on 'Resend Confirmation' in the upper bar in order to re-send the confirmation e-mail.",
      options
    )
