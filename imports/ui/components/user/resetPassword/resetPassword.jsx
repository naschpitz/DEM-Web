...
export default (props) => {
  ...
  function onFormSubmit(event) {
    ...
    function callback(error) {
      ...
      else resetMsgId = messageDisplay.show("success", "Password changed successfully.", resetMsgId)
  ...
  return (
    <form id="form-resetPassword" onSubmit={onFormSubmit}>
      ...
      <button id="btnReset" className="btn btn-primary btn-sm btn-block" type="submit" disabled={!canReset()}>
        {isResettingPassword ? <div>Resetting password...</div> : <div>Reset Password</div>}
      </button>
