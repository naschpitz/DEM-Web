...
export default Login = props => {
  ...
  function onFormSubmit(event) {
    ...
    function callback(error) {
      if (error) loginMsgId = messageDisplay.show("error", "Error loggin in: " + getErrorMessage(error), loginMsgId)
      else {
  ...
  function onForgotPasswordClick() {
    ...
    if (!email) {
      forgotMsgId = messageDisplay.show(
        "error",
        "Field 'E-mail' empty. Type in the e-mail that you registered your account in order to recover the password.",
        forgotMsgId
      )
      return
    }
    ...
    function callback(error) {
      if (error) forgotMsgId = messageDisplay.show("error", "E-mail not registered in the database.", forgotMsgId)
      else
        forgotMsgId = messageDisplay.show(
          "success",
          "An e-mail for password recovery has been sent to the registered address.",
          forgotMsgId
        )
    ...
    <form id="form-login" onSubmit={onFormSubmit}>
      ...
      <div id="blockForgotPassword">
        {isSendingRecovery ? (
          ...
          <a href="#" onClick={onForgotPasswordClick} id="forgotPassword">
            Forgot my password
          </a>
        )}
      </div>
      ...
      <div id="blockClose">
        <button className="btn btn-danger btn-sm" onClick={onCloseClick}>
