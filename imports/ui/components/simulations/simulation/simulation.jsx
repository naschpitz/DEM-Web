...
export default Simulation = (props) => {
    ...
    const MenuLink = ({tab, label}) => {
        ...
        return (
            <li className="nav-item">
                <Link className={'nav-link ' + (active ? 'active' : '')} to={to} role="tab">{label}</Link>
            </li>
        );
    };
    ...
    function changeUrl(object) {
        ...
        if (object.tab && object.tab !== currentTab) { setCurrentTab(object.tab); }
    ...
    function renderTab() {
        ...
        switch (tab) {
            case 'main': return <Main name="main" simulationId={simulationId} />;
            case 'calibration': return <Calibration name="calibration" simulationId={simulationId}/>;
        }
    }
